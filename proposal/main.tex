...
\newcommand{\myacro}[0]{\replaced[id=PL]{FULLMAP}{XXXX}}
\newcommand{\mytitle}[0]{\myacro: \\ \deleted[id=PL, comment={Je suis de même avis que Tanguy, ça fait "plouc"}]{Easy} \replaced[id=PL, comment={Une proposition avec un acronyme}]{Faust Ultra Low Latency Multichannel Audio Processing}{High-Performance Ultra-Low-Latency Multichannel Audio Processing and Synthesis}}
...
\end{itemize}
%\PP{} % issue with capitalization
Our project will gather the strengths of GRAME-CNCM\footnote{\url{http://grame.fr}} (\F{}, compilation, digital lutherie, musical audio applications, dissemination/mediation, professional music production), CITI\footnote{\url{http://www.citi-lab.fr}} at INSA Lyon (FPGA, DSP, fixed-point processing with FloPoCo\footnote{\url{http://flopoco.gforge.inria.fr}}), and LMFA\footnote{\url{http://lmfa.ec-lyon.fr}} at École Centrale de Lyon (acoustics, DSP, artificial reverberation, active control).

Real-time audio DSP is used at the core of many systems ranging from musical instruments to \replaced[id=PL, comment={ou bien Active Noise Control (ANC), c'est assez utilisé dans la littérature, même audio}]{active control of sound}{active acoustics correction}\cite{nelson1991activea} technologies \deleted[id=PL]{, speakers, etc}. Latency plays a crucial role in this context.  In the domain of music, wether we're talking about a digital mixing desk, a musical instrument (e.g., synthesizers, effects/guitar pedals, etc.), or a digital speaker or microphone, the perception of latency can have a significant impact on playability \cite{Lago2004}. In the domain of \replaced[id=PL]{active control of sound}{active control applied to acoustics} where it is typically necessary to ``beat'' acoustical waves by processing signals faster than the speed of sound, low latency is a determining factor \replaced[id=PL, comment={J'ai changé la ref vers un bouquin et la ref plutôt pour l'aspect multicanal et reverb modale}]{\cite{elliott2000signal}}{\cite{Zhang2018}}.
...
\added[id=PL,comment={J'ai rajouté un paragraphe sur l'aspect huge amount of GPIO pour les FPGA, qui me semble être un atout essentiel de ce genre de chip pour le multiplexage massif de données, le traitement multicanal à grand nombre de voies, etc.}]{Another advantage of FPGAs is their large number of GPIOs. It then becomes possible to implement modern multi-channel processing algorithms, which are difficult to implement in practice otherwise. For example, for precise measurement and characterization of sound fields using a very large number of digital microphones \cite{salze2019new} or for active sound control over a large region of space where additionnal loudspeakers are involved  \cite{Zhang2018}} 
...
% F2D: j'ai ajouté le mot "open": je ne sais pas si c'est votre intention.
...
Another of the main goals of \PP{} is to \textbf{develop a new \F{} architecture back-end for FPGA-based platforms}.
%building upon the successful experiments developed in the previous SyFaLa collaborative project, funded by the \emph{Fédération d'Informatique de Lyon}.
...
\highlight[id=PL, comment={Je vois avec Sébastien pour un texte accrocheur, je n'ai pas encore 15 ans d'expériene au labo... :)}]{TODO: something about LMFA.}
...
Possibilities unveiled by these developments will impact a wide range of fields and could lead to plethora of technological and industrial applications to design concert halls of a new kind, suppress noise in a car's passenger compartment, diminish vibrations in airplane jet engines, improve existing digital musical instrument technologies, etc. \highlight[id=PL,comment={Je pense que ce paragraphe irait bien au tout début, révolutionner le DSP audio et toutes les conséquences que ça peut avoir..}]{}
