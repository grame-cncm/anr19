...
\added[id=PL, comment={J'essai un petit texte d'intro pour situer le contexte du projet, introduire le verrou scientifique et les objectifs pour le lever, à retravailler si besoin, évidemment.}]{
%Audio Digital Signal Processing (DSP) is used everywhere in nowadays life, in a multitude of embedded systems (TV, cars, speakers, smartphone, tablets, etc.) It is an intrinsic part of the digital revolution in our modern societies.
In the art and culture sectors, embedded systems for audio and multimedia are increasingly used (interactive systems, musical instruments, virtual and augmented reality, artistic creation tools, musical composition and performance, etc.) However, the programming and realization of such systems are still difficult for artists, creators or engineers not deticated to software/hardware implementation. 
Over the past decade, progress has been made to facilitate this implementation, offering more flexibility in digital approaches to artistic creation. For instance, the Arduinos platforms %\footnote{\url{https://www.arduino.cc}}
...
But many limitations remain with microcontroller-based approaches, especially for real-time applications where latency plays a crucial role (e.g. active control of sound \cite{elliott2000signal}, digital instruments playability \cite{Lago2004}, digital audio effects, etc.).
In fact, audio processing latency can be reduced on ``standard'' computing platforms (i.e., personal computers based on CPUs (Central Processing Unit) by using dedicated hardware (i.e., audio interfaces, etc.) and software (i.e., low-latency Linux kernels, etc.) solutions, but, buffering remains a limiting factor and going under the ``one millisecond threshold'' (which corresponds to a distance of about 35cm in the air) is usually impossible.
That is where FPGA (Field Programmable Gate Arrays) can help this field to a new level. Unlike microcontrollers, these hardware chips are known for their high computational capabilities \cite{Choi2013,Pfeifle2012} and very low-latency performances \cite{Verstraelen2014}.
}
They start to appear for real-time audio DSP applications, both in the industry (i.e., Antelope Audio,\footnote{\url{https://en.antelopeaudio.com}} Korora Audio,\footnote{\url{https://www.kororaaudio.com}} etc.) and in academia. %Similarly, FPGA-based boards specifically targeting audio applications such as Dada Machines' Doppler\footnote{\url{https://dadamachines.com/product/doppler}} are starting to appear.
...
Therefore, FPGA-based solutions seem suitable for a very large number of artistic and cultural applications in the era of the digital revolution, but at the moment it is totally inaccessible to this whole community because FPGAs are as well famous for their very complex programming.

In this context, the FAST project is designed to overcome the difficulties mentioned above (i.e. low latency, computing capacity, ease of programming) and offer the greatest number of people, particularly artists and musicians, the opportunity of bringing to the real world their creations.
